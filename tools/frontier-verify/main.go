...
func init() {
	rootCmd.PersistentFlags().StringVarP(&frontierURL, "url", "u", "", "Frontier server URL")
...
var rootCmd = &cobra.Command{
	Use:   "frontier-verify",
	Short: "tool to check frontier data consistency",
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("%s: Checking %d ledgers starting from cursor \"%s\"\n\n", frontierURL, count, ledgerCursor)

		for {
			ledgersPage, err := client.Ledgers(frontierclient.LedgerRequest{
				Limit:  200,
			...
			for _, ledger := range ledgersPage.Embedded.Records {
				...
				transactionsPage, err := client.Transactions(frontierclient.TransactionRequest{
				...
				for _, transaction := range transactionsPage.Embedded.Records {
					...
					go func(transaction protocol.Transaction) {
						...
						operationsPage, err := client.Operations(frontierclient.OperationRequest{
