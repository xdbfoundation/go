...
func checkPaths(paths []string) {
	for _, path := range paths {
		a := cmp.NewResponse(frontierBase, path, false)
		b := cmp.NewResponse(frontierTest, path, false)
...
func getAllPagesPaths(page string) []string {
	...
	for {
		resp, err := http.Get(frontierBase + page)
		if err != nil {
			panic(err)
		}
		...
		page = strings.Replace(page, frontierBase, "", -1)
		...
		// no extra objects in the Frontier we are testing.
