...
var rootCmd = &cobra.Command{
	Use:   "frontier-cmp",
	Short: "frontier-cmp compares two frontier servers' responses",
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&frontierBase, "base", "b", "", "URL of the base/old version Frontier server")
	rootCmd.PersistentFlags().StringVarP(&frontierTest, "test", "t", "", "URL of the test/new version Frontier server")
...
func run(cmd *cobra.Command) {
	...
	ledger := getLatestLedger(frontierBase)
	...
	outputDir := fmt.Sprintf("%s/frontier-cmp-diff/%s", pwd, time.Now().Format(timeFormat))
	...
	for {
		...
		go func() {
			...
			go func() {
				a = cmp.NewResponse(frontierBase, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			go func() {
				b = cmp.NewResponse(frontierTest, pl.Path, pl.Stream)
				requestWg.Done()
			}()
			...
			// older Frontier versions don't send this header.
...
func getLatestLedger(url string) protocol.Ledger {
	...
	ledgers, err := frontier.Ledgers(client.LedgerRequest{
...
func addPathsFromResponse(a *cmp.Response, level int) {
	...
	for _, newPath := range newPaths {
		...
		// without cursor. There will always be a diff if Frontier started
