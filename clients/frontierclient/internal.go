...
// decodeResponse decodes the response from a request to a frontier server
func decodeResponse(resp *http.Response, object interface{}, hc *Client) (err error) {
	defer resp.Body.Close()
	decoder := json.NewDecoder(resp.Body)

	u, err := url.Parse(hc.FrontierURL)
	if err != nil {
		return errors.Errorf("unable to parse the provided frontier url: %s", hc.FrontierURL)
	}
	...
	if !(resp.StatusCode >= 200 && resp.StatusCode < 300) {
		...
		decodeError := decoder.Decode(&frontierError.Problem)
		if decodeError != nil {
			return errors.Wrap(decodeError, "error decoding frontier.Problem")
		}
...
// setCurrentServerTime saves the current time returned by a frontier server
...
// currentServerTime returns the current server time for a given frontier server
