...
func (c *Client) sendRequest(hr FrontierRequest, resp interface{}) (err error) {
	endpoint, err := hr.BuildURL()
	if err != nil {
		return
	}

	c.FrontierURL = c.fixFrontierURL()
	_, ok := hr.(submitRequest)
	if ok {
		return c.sendRequestURL(c.FrontierURL+endpoint, "post", resp)
	}

	return c.sendRequestURL(c.FrontierURL+endpoint, "get", resp)
}

// checkMemoRequired implements a memo required check as defined in
// https://github.com/digitalbits/digitalbits-protocol/blob/master/ecosystem/sep-0029.md
func (c *Client) checkMemoRequired(transaction *txnbuild.Transaction) error {
	...
	for i, op := range transaction.Operations() {
		...
		if err != nil {
			frontierError := GetError(err)

			if frontierError == nil || frontierError.Response.StatusCode != 404 {
				return err
			}
...
// sendRequestURL sends a url to a frontier server.
// It can be used for requests that do not implement the FrontierRequest interface.
func (c *Client) sendRequestURL(requestURL string, method string, a interface{}) (err error) {
	...
	c.setDefaultClient()
	if c.frontierTimeout == 0 {
		c.frontierTimeout = FrontierTimeout
	}
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*c.frontierTimeout)
...
// stream handles connections to endpoints that support streaming on a frontier server
			...
			for {
				...
				if err != nil {
					if err == io.EOF || err == io.ErrUnexpectedEOF {
						...
						//   happen in Frontier as it always sends an empty line after each event.
						// - The stream was closed by the server/proxy because the connection was idle.
						//
						// In the former case, that (again) should never happen in Frontier, we need to
						// check if there are any events we need to decode. We do this in the `if`
						// statement below just in case if Frontier behaviour changes in a future.
						//
						// From spec:
...
func (c *Client) setClientAppHeaders(req *http.Request) {
	req.Header.Set("X-Client-Name", "go-digitalbits-sdk")
...
// fixFrontierURL strips all slashes(/) at the end of FrontierURL if any, then adds a single slash
func (c *Client) fixFrontierURL() string {
	return strings.TrimRight(c.FrontierURL, "/") + "/"
}

// SetFrontierTimeout allows users to set the timeout before a frontier request is cancelled.
// The timeout is specified as a time.Duration which is in nanoseconds.
func (c *Client) SetFrontierTimeout(t time.Duration) *Client {
	c.frontierTimeout = t
	return c
}

// FrontierTimeout returns the current timeout for a frontier client
func (c *Client) FrontierTimeout() time.Duration {
	return c.frontierTimeout
}

// Accounts returns accounts who have a given signer or
// have a trustline to an asset.
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/accounts.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/accounts-single.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/data-for-account.html
...
// Effects returns effects(https://www.digitalbits.org/developers/frontier/reference/resources/effect.html)
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/assets-all.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/ledgers-all.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/ledgers-single.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/fee-stats.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/offers-for-account.html
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/offer-details.html
...
// Operations returns digitalbits operations (https://www.digitalbits.org/developers/frontier/reference/resources/operation.html)
...
// OperationDetail returns a single digitalbits operation for a given operation id
// See https://www.digitalbits.org/developers/frontier/reference/resources/operation.html
func (c *Client) OperationDetail(id string) (ops operations.Operation, err error) {
	...
	if err != nil {
		return ops, errors.Wrap(err, "sending request to frontier")
	}
...
// SubmitTransactionXDR submits a transaction represented as a base64 XDR string to the network. err can be either error object or frontier.Error object.
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/transactions-create.html
...
// error object or a frontier.Error object.
//
// This function will always check if the destination account requires a memo in the transaction as
// defined in SEP0029: https://github.com/digitalbits/digitalbits-protocol/blob/master/ecosystem/sep-0029.md
//
...
// you to pass SubmitTxOpts. err can be either an error object or a frontier.Error object.
//
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/transactions-create.html
...
// Transactions returns digitalbits transactions (https://www.digitalbits.org/developers/frontier/reference/resources/transaction.html)
...
// See https://www.digitalbits.org/developers/frontier/reference/endpoints/transactions-single.html
...
// OrderBook returns the orderbook for an asset pair (https://www.digitalbits.org/developers/frontier/reference/resources/orderbook.html)
func (c *Client) OrderBook(request OrderBookRequest) (obs hProtocol.OrderBookSummary, err error) {
	err = c.sendRequest(request, &obs)
	return
}

// Paths returns the available paths to make a strict receive path payment. See https://www.digitalbits.org/developers/frontier/reference/endpoints/path-finding-strict-receive.html
...
// StrictReceivePaths returns the available paths to make a strict receive path payment. See https://www.digitalbits.org/developers/frontier/reference/endpoints/path-finding-strict-receive.html
func (c *Client) StrictReceivePaths(request PathsRequest) (paths hProtocol.PathsPage, err error) {
	err = c.sendRequest(request, &paths)
	return
}

// StrictSendPaths returns the available paths to make a strict send path payment. See https://www.digitalbits.org/developers/frontier/reference/endpoints/path-finding-strict-send.html
func (c *Client) StrictSendPaths(request StrictSendPathsRequest) (paths hProtocol.PathsPage, err error) {
	err = c.sendRequest(request, &paths)
	return
}

// Payments returns digitalbits account_merge, create_account, path payment and payment operations.
...
// Trades returns digitalbits trades (https://www.digitalbits.org/developers/frontier/reference/resources/trade.html)
...
// https://www.digitalbits.org/developers/guides/get-started/create-account.html for more information.
func (c *Client) Fund(addr string) (tx hProtocol.Transaction, err error) {
	...
	friendbotURL := fmt.Sprintf("%sfriendbot?addr=%s", c.fixFrontierURL(), addr)
...
// TradeAggregations returns digitalbits trade aggregations (https://www.digitalbits.org/developers/frontier/reference/resources/trade_aggregation.html)
...
// StreamEffects streams frontier effects. It can be used to stream all effects or account specific effects.
...
// StreamOperations streams digitalbits operations. It can be used to stream all operations or operations
...
// StreamPayments streams digitalbits payments. It can be used to stream all payments or payments
...
// StreamOffers streams offers processed by the DigitalBits network for an account. Use context.WithCancel
...
// StreamLedgers streams digitalbits ledgers. It can be used to stream all ledgers. Use context.WithCancel
// to stop streaming or context.Background() if you want to stream indefinitely.
...
// FetchTimebounds provides timebounds for N seconds from now using the server time of the frontier instance.
...
func (c *Client) FetchTimebounds(seconds int64) (txnbuild.Timebounds, error) {
	serverURL, err := url.Parse(c.FrontierURL)
	if err != nil {
		return txnbuild.Timebounds{}, errors.Wrap(err, "unable to parse frontier url")
	}
...
// Root loads the root endpoint of frontier
func (c *Client) Root() (root hProtocol.Root, err error) {
	err = c.sendRequestURL(c.fixFrontierURL(), "get", &root)
	return
}
