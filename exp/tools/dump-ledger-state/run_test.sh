...
if [ -z ${TESTNET+x} ]; then
    ...
    digitalbits-core --conf ./digitalbits-core.cfg catchup $LATEST_LEDGER/1
else
    echo "Catch up from testnet"
    digitalbits-core --conf ./digitalbits-core-testnet.cfg catchup $LATEST_LEDGER/1
fi

echo "Dumping state from digitalbits-core..."
./dump_core_db.sh
echo "State dumped..."
