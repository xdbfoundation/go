...
type digitalbitsCoreRunnerMode int

const (
	digitalbitsCoreRunnerModeOnline digitalbitsCoreRunnerMode = iota
	digitalbitsCoreRunnerModeOffline
)

// digitalbitsCoreRunner uses a named pipe ( https://en.wikipedia.org/wiki/Named_pipe ) to stream ledgers directly
// from DigitalBits Core
type pipe struct {
	// digitalbitsCoreRunner will be reading ledgers emitted by DigitalBits Core from the pipe.
	// After the DigitalBits Core process exits, digitalbitsCoreRunner should eventually close the reader.
	Reader io.ReadCloser
	// digitalbitsCoreRunner is responsible for closing the named pipe file after the DigitalBits Core process exits.
	// However, only the DigitalBits Core process will be writing to the pipe. digitalbitsCoreRunner should not
	// write anything to the named pipe file which is why the type of File is io.Closer.
	File io.Closer
}

type digitalbitsCoreRunner struct {
...
func newDigitalBitsCoreRunner(config CaptiveCoreConfig, mode digitalbitsCoreRunnerMode) (*digitalbitsCoreRunner, error) {
	// Create temp dir
	tempDir, err := ioutil.TempDir("", "captive-digitalbits-core")
	if err != nil {
...
func (r *digitalbitsCoreRunner) generateConfig() (string, error) {
	if r.mode == digitalbitsCoreRunnerModeOnline && r.configAppendPath == "" {
		return "", errors.New("digitalbits-core append config file path cannot be empty in online mode")
	}
	lines := []string{
	...
	if r.mode == digitalbitsCoreRunnerModeOffline {
	...
	if r.mode == digitalbitsCoreRunnerModeOffline && r.configAppendPath == "" {
...
func (r *digitalbitsCoreRunner) getConfFileName() string {
	return filepath.Join(r.tempDir, "digitalbits-core.conf")
}

func (r *digitalbitsCoreRunner) getLogLineWriter() io.Writer {
	rd, wr := io.Pipe()
	br := bufio.NewReader(rd)

	// Strip timestamps from log lines from captive digitalbits-core. We emit our own.
...
// platform-specific captiveDigitalBitsCore.Start() methods.
func (r *digitalbitsCoreRunner) writeConf() error {
...
func (r *digitalbitsCoreRunner) createCmd(params ...string) *exec.Cmd {
...
func (r *digitalbitsCoreRunner) runCmd(params ...string) error {
	cmd := r.createCmd(params...)
	defer r.closeLogLineWriters(cmd)

	if err := cmd.Start(); err != nil {
		return errors.Wrapf(err, "could not start `digitalbits-core %v` cmd", params)
	}

	if err := cmd.Wait(); err != nil {
		return errors.Wrapf(err, "error waiting for `digitalbits-core %v` subprocess", params)
	}
	return nil
}

// context returns the context.Context instance associated with the running captive core instance
func (r *digitalbitsCoreRunner) context() context.Context {
	return r.ctx
}

// catchup executes the catchup command on the captive core subprocess
func (r *digitalbitsCoreRunner) catchup(from, to uint32) error {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if err := r.runCmd("new-db"); err != nil {
		return errors.Wrap(err, "error waiting for `digitalbits-core new-db` subprocess")
	}
	...
	if err != nil {
		...
		return errors.Wrap(err, "error starting `digitalbits-core catchup` subprocess")
	}
...
func (r *digitalbitsCoreRunner) runFrom(from uint32, hash string) error {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	if err != nil {
		r.closeLogLineWriters(cmd)
		return errors.Wrap(err, "error starting `digitalbits-core run` subprocess")
	}
...
func (r *digitalbitsCoreRunner) handleExit(cmd *exec.Cmd) {
	defer r.wg.Done()
	exitErr := cmd.Wait()
	r.closeLogLineWriters(cmd)
...
func (r *digitalbitsCoreRunner) closeLogLineWriters(cmd *exec.Cmd) {
...
func (r *digitalbitsCoreRunner) getMetaPipe() <-chan metaResult {
...
func (r *digitalbitsCoreRunner) getProcessExitError() (bool, error) {
	r.lock.Lock()
	defer r.lock.Unlock()
...
func (r *digitalbitsCoreRunner) close() error {
	r.lock.Lock()
	started := r.started
	tempDir := r.tempDir
	...
	if started {
		// wait for the digitalbits core process to terminate
		r.wg.Wait()
