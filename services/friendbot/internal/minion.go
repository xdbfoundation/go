...
// Minion contains a DigitalBits channel account and Go channels to communicate with friendbot.
type Minion struct {
	Account         Account
	Keypair         *keypair.Full
	BotAccount      txnbuild.Account
	BotKeypair      *keypair.Full
	Frontier         *frontierclient.Client
	Network         string
	StartingBalance string
	BaseFee         int64

	// Mockable functions
	SubmitTransaction    func(minion *Minion, hclient *frontierclient.Client, tx string) (*hProtocol.Transaction, error)
	CheckSequenceRefresh func(minion *Minion, hclient *frontierclient.Client) error
...
func (minion *Minion) Run(destAddress string, resultChan chan SubmitResult) {
	err := minion.CheckSequenceRefresh(minion, minion.Frontier)
	if err != nil {
	...
	succ, err := minion.SubmitTransaction(minion, minion.Frontier, txStr)
	resultChan <- SubmitResult{
...
func SubmitTransaction(minion *Minion, hclient *frontierclient.Client, tx string) (*hProtocol.Transaction, error) {
	result, err := hclient.SubmitTransactionXDR(tx)
	if err != nil {
		errStr := "submitting tx to frontier"
		switch e := err.(type) {
		case *frontierclient.Error:
			minion.checkHandleBadSequence(e)
			resStr, resErr := e.ResultString()
			if resErr != nil {
				errStr += ": error getting frontier error code: " + resErr.Error()
			} else if resStr == createAccountAlreadyExistXDR {
				return nil, errors.Wrap(ErrAccountExists, errStr)
			} else {
				errStr += ": frontier error string: " + resStr
			}
...
func CheckSequenceRefresh(minion *Minion, hclient *frontierclient.Client) error {
...
func (minion *Minion) checkHandleBadSequence(err *frontierclient.Error) {
