...
// to a digitalbits-core instance.
type System struct {
	...
	DB                func(context.Context) FrontierDB
	...
	Metrics struct {
		...
		// submissions to digitalbits-core
...
func (sys *System) waitUntilAccountSequence(ctx context.Context, db FrontierDB, sourceAddress string, seq uint64) bool {
...
func (sys *System) Init() {
	sys.initializer.Do(func() {
		...
		if sys.SubmissionTimeout == 0 {
			...
			// timeout.
			// 30 seconds is 6 ledgers (with avg. close time = 5 sec), enough for digitalbits-core
