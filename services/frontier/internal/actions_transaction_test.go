...
func TestTransactionActions_Show(t *testing.T) {
	...
	if ht.Assert.Equal(200, w.Code) {
		var actual frontier.Transaction
...
func TestTransactionActions_Show_Failed(t *testing.T) {
	...
	// NULL value
	_, err := ht.FrontierSession().ExecRaw(
...
func TestTransactionActions_Index(t *testing.T) {
	...
	// regression: https://github.com/xdbfoundation/go/services/frontier/internal/issues/365
	w = ht.Get("/transactions?limit=200")
...
func TestPostFeeBumpTransaction(t *testing.T) {
	...
	defer ht.Finish()
	test.ResetFrontierDB(t, ht.FrontierDB)
	q := &history.Q{ht.FrontierSession()}
	...
	w := ht.Post("/transactions", form)
	ht.Assert.Equal(200, w.Code)
	var response frontier.Transaction
