...
func TestTradeActions_Index(t *testing.T) {
	...
	defer ht.Finish()
	var records []frontier.Trade
	var firstTrade frontier.Trade
	...
	if ht.Assert.Equal(200, w.Code) {
		...
		// 	ensure created_at is populated correctly
		l := history.Ledger{}
		hq := history.Q{Session: ht.FrontierSession()}
...
func testTradeAggregationPrices(t *HTTPT, record frontier.TradeAggregation) {
...
func TestTradeActions_Aggregation(t *testing.T) {
	...
	var records []frontier.TradeAggregation
	var record frontier.TradeAggregation
	var nextLink string
...
func TestTradeActions_AmountsExceedInt64(t *testing.T) {
	...
	for i := 1; i <= 3; i++ {
		timestamp := digitalbitsTime.MillisFromInt64(start + (minute * int64(i-1)))
		err := IngestTestTrade(
...
func TestTradeActions_IndexRegressions(t *testing.T) {
	...
	t.Run("Regression for nil prices: https://github.com/digitalbits/go/issues/357", func(t *testing.T) {
		...
		_ = ht.FrontierDB.MustExec("UPDATE history_trades SET price_n = NULL, price_d = NULL")
		w = ht.Get("/trades")
