...
func TestOperationIncludeTransactions(t *testing.T) {
	...
	withoutTransactionsQuery := (&Q{tt.FrontierSession()}).Operations().
		ForAccount(accountID)

	var expectedTransactions []Transaction
	err = (&Q{tt.FrontierSession()}).Transactions().ForAccount(accountID).Select(&expectedTransactions)
	tt.Assert.NoError(err)
