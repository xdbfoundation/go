...
func TestRootAction(t *testing.T) {
	...
	ht.App.config.DigitalBitsCoreURL = server.URL
	ht.App.config.NetworkPassphrase = "test"
	ht.App.UpdateDigitalBitsCoreInfo()
	ht.App.UpdateLedgerState()

	w := ht.Get("/")

	if ht.Assert.Equal(200, w.Code) {
		var actual frontier.Root
		err := json.Unmarshal(w.Body.Bytes(), &actual)
		ht.Require.NoError(err)
		ht.Assert.Equal("devel", actual.FrontierVersion)
		ht.Assert.Equal("test-core", actual.DigitalBitsCoreVersion)
