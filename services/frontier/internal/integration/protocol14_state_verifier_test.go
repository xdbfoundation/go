...
func TestProtocol14StateVerifier(t *testing.T) {
	...
	err = itest.Frontier().HistoryQ().UpdateExpIngestVersion(0)
	assert.NoError(t, err)
...
func waitForStateVerifications(itest *integration.Test, count int) bool {
	...
	for i := 0; i < 120; i++ {
		...
		stateInvalid := strings.Contains(metrics, "frontier_ingest_state_invalid 1")
		assert.False(t, stateInvalid, "State is invalid!")

		notVerifiedYet := strings.Contains(
			metrics,
			fmt.Sprintf("frontier_ingest_state_verify_duration_seconds_count %d", count-1),
		)
		if notVerifiedYet {
