...
func mustNewDBSession(databaseURL string, maxIdle, maxOpen int) *db.Session {
	...
	if err != nil {
		log.Fatalf("cannot open Frontier DB: %v", err)
	}
...
func mustInitFrontierDB(app *App) {
	maxIdle := app.config.FrontierDBMaxIdleConnections
	maxOpen := app.config.FrontierDBMaxOpenConnections
	if app.config.Ingest {
		maxIdle -= ingest.MaxDBConnections
		maxOpen -= ingest.MaxDBConnections
		if maxIdle <= 0 {
			log.Fatalf("max idle connections to frontier db must be greater than %d", ingest.MaxDBConnections)
		}
		if maxOpen <= 0 {
			log.Fatalf("max open connections to frontier db must be greater than %d", ingest.MaxDBConnections)
		}
	}
...
func initExpIngester(app *App) {
	...
	if !app.config.EnableCaptiveCoreIngestion {
		coreSession = mustNewDBSession(app.config.DigitalBitsCoreDatabaseURL, ingest.MaxDBConnections, ingest.MaxDBConnections)
	}
	app.ingester, err = ingest.NewSystem(ingest.Config{
		...
		DigitalBitsCoreURL:              app.config.DigitalBitsCoreURL,
		DigitalBitsCoreCursor:           app.config.CursorName,
...
func initPathFinder(app *App) {
	...
	app.orderBookStream = ingest.NewOrderBookStream(
		&history.Q{app.FrontierSession(app.ctx)},
		orderBookGraph,
	)
...
func initDbMetrics(app *App) {
	...
	app.historyLatestLedgerClosedAgoGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			Namespace: "frontier", Subsystem: "history", Name: "latest_ledger_closed_ago_seconds",
	...
	app.historyElderLedgerCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{Namespace: "frontier", Subsystem: "history", Name: "elder_ledger"},
		func() float64 {
			ls := app.ledgerState.CurrentStatus()
	...
	app.coreLatestLedgerCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{Namespace: "frontier", Subsystem: "digitalbits_core", Name: "latest_ledger"},
		func() float64 {
			ls := app.ledgerState.CurrentStatus()
	...
	app.dbMaxOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "frontier", Subsystem: "db", Name: "max_open_connections"},
		func() float64 {
			// Right now MaxOpenConnections in Frontier is static however it's possible that
	...
	app.dbOpenConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "frontier", Subsystem: "db", Name: "open_connections"},
		func() float64 {
	...
	app.dbInUseConnectionsGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{Namespace: "frontier", Subsystem: "db", Name: "in_use_connections"},
		func() float64 {
	...
	app.dbWaitCountCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "frontier", Subsystem: "db", Name: "wait_count_total",
	...
	app.dbWaitDurationCounter = prometheus.NewCounterFunc(
		prometheus.CounterOpts{
			Namespace: "frontier", Subsystem: "db", Name: "wait_duration_seconds_total",
...
func initSubmissionSystem(app *App) {
	app.submitter = &txsub.System{
		...
		Submitter:       txsub.NewDefaultSubmitter(http.DefaultClient, app.config.DigitalBitsCoreURL),
		SubmissionQueue: sequence.NewManager(),
		DB: func(ctx context.Context) txsub.FrontierDB {
			return &history.Q{Session: app.FrontierSession(ctx)}
		},
	}
}
