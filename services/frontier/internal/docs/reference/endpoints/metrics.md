...
Frontier utilizes Go's built in concurrency primitives ([goroutines](https://gobyexample.com/goroutines) and [channels](https://gobyexample.com/channels)). This metric monitors the number of currently running goroutines on this Frontier's process.
...
Frontier maintains its own database (postgres), a verbose and user friendly account of activity on the DigitalBits network.

|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| elder_ledger     | The sequence number of the oldest ledger recorded in Frontier's database. |
| latest_ledger    | The sequence number of the youngest (most recent) ledger recorded in Frontier's database.  |
| open_connections | The number of open connections to the Frontier database. |
...
#### Ingester
Ingester represents metrics specific to Frontier's [ingestion](https://github.com/xdbfoundation/go/blob/master/services/frontier/internal/docs/admin.md#ingesting-live-digitalbits-core-data) process, or the process by which Frontier consumes transaction results from a connected DigitalBits Core instance.

|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| clear_ledger |  The count and rate of clearing (per ledger) for this Frontier process.  |
| ingest_ledger | The count and rate of ingestion (per ledger)  for this Frontier process. |
...
Frontier utilizes the standard `debug`, `error`, etc. levels of logging. This metric outputs stats for each level of log message produced, useful for a high-level monitoring of "is my Frontier instance functioning properly?" In order of increasing severity:
...
Requests represents an overview of Frontier's incoming traffic.
...
#### DigitalBits Core
As noted above, Frontier relies on DigitalBits Core to stay in sync with the DigitalBits network. These metrics are specific to the underlying DigitalBits Core instance.

|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| latest_ledger    | The sequence number of the latest (most recent) ledger recorded in DigitalBits Core's database.  |
| open_connections | The number of open connections to the DigitalBits Core postgres database.  |
...
Frontier does not submit transactions directly to the DigitalBits network. Instead, it sequences transactions and sends the base64 encoded, XDR serialized blob to its connected DigitalBits Core instance. 
...
|    Metric     |  Description                                                                                                                               |
| ---------------- |  ------------------------------------------------------------------------------------------------------------------------------ |
| buffered | The count of submissions buffered behind this Frontier's submission queue.  |
| failed | The rate of failed transactions that have been submitted to this Frontier.  |
| open |The count of "open" submissions (i.e.) submissions whose transactions haven't been confirmed successful or failed.  |
| succeeded | The rate of successful transactions that have been submitted to this Frontier.  |
| total | Both the rate and count of all transactions submitted to this Frontier. |
