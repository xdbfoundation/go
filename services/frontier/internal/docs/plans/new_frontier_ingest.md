...
1. A History Archive <sup>[1](https://github.com/xdbfoundation/DigitalBits/blob/master/docs/software/admin.md#history-archives),[2](https://github.com/xdbfoundation/DigitalBits/blob/master/docs/history.md)</sup>, which is generated by `digitalbits-core` and provides a complete copy of a recent [ledger](https://github.com/xdbfoundation/docs/blob/master/guides/concepts/ledger.md) as well as a history of `TransactionResult` XDR objects, which in turn contain [operation results](https://github.com/xdbfoundation/DigitalBits/blob/master/src/xdr/DigitalBits-transaction.x#L382-L834).
2. A full ledger reader, which provides random and up-to-date access to the transaction sets & [`TransactionMeta`](https://github.com/xdbfoundation/DigitalBits/blob/master/src/xdr/DigitalBits-ledger.x#L271-L318) in each ledger close. `TransactionMeta` info is essential for keeping a view of the ledger state up to date, as it describes the changes to ledger state that result from each operation.
...
1. The ingestion does not put load on any external service (like XDB Foundation history archives)
...
However, this requires a change to `digitalbits-core`, which is estimated to happen in Q3 2019. Until then, the ingestion system will read from the `txfeehistory` and [`txhistory`](https://github.com/xdbfoundation/DigitalBits/blob/master/src/transactions/TransactionFrame.cpp#L683) tables in the `digitalbits-core` database as it does currently. Unfortunately, we won't get any of the benefits listed above until the change is made to `digitalbits-core`.
