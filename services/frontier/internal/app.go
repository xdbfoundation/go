...
import (
	...
	"github.com/xdbfoundation/go/services/frontier/internal/reap"
...
// Serve starts the frontier web server, binding it to a socket, setting up
// the shutdown signals.
func (a *App) Serve() {

	log.Infof("Starting frontier on :%d (ingest: %v)", a.config.Port, a.config.Ingest)
...
// history portion of frontier's database.
func (a *App) HistoryQ() *history.Q {
	return a.historyQ
}

// Ingestion returns the ingestion system associated with this Frontier instance
func (a *App) Ingestion() ingest.System {
	return a.ingester
}

// FrontierSession returns a new session that loads data from the frontier
// database. The returned session is bound to `ctx`.
func (a *App) FrontierSession(ctx context.Context) *db.Session {
...
func (a *App) UpdateLedgerState() {
	...
	coreClient := &digitalbitscore.Client{
	...
	if err != nil {
		logErr(err, "failed to load the digitalbits-core info")
		return
	}
...
// UpdateDigitalBitsCoreInfo updates the value of CoreVersion,
// CurrentProtocolVersion, and CoreSupportedProtocolVersion from the DigitalBits
// core API.
func (a *App) UpdateDigitalBitsCoreInfo() {
	if a.config.DigitalBitsCoreURL == "" {
		return
	}
	...
	if err != nil {
		log.Warnf("could not load digitalbits-core info: %s", err)
		return
	}

	// Check if NetworkPassphrase is different, if so exit Frontier as it can break the
	// state of the application.
	if resp.Info.Network != a.config.NetworkPassphrase {
		log.Errorf(
			"Network passphrase of digitalbits-core (%s) does not match Frontier configuration (%s). Exiting...",
			resp.Info.Network,
...
// Tick triggers frontier to update all of it's background processes such as
// transaction submission, metrics, ingestion and reaping.
func (a *App) Tick() {
	var wg sync.WaitGroup
	log.Debug("ticking app")
	// update ledger state, operation fee state, and digitalbits-core info in parallel
	wg.Add(3)
	go func() { a.UpdateLedgerState(); wg.Done() }()
	go func() { a.UpdateFeeStatsState(); wg.Done() }()
	go func() { a.UpdateDigitalBitsCoreInfo(); wg.Done() }()
	wg.Wait()
...
func (a *App) init() error {
	...
	// digitalbitsCoreInfo
	a.UpdateDigitalBitsCoreInfo()

	// frontier-db and core-db
	mustInitFrontierDB(a)
	...
	// reaper
	a.reaper = reap.New(a.config.HistoryRetentionCount, a.FrontierSession(context.Background()), a.ledgerState)
