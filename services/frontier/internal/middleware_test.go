...
func TestStateMiddleware(t *testing.T) {
	...
	endpoint := func(w http.ResponseWriter, r *http.Request) {
		session := r.Context().Value(&frontierContext.SessionContextKey).(*db.Session)
	...
	stateMiddleware := &httpx.StateMiddleware{
		FrontierSession: tt.FrontierSession(),
	}
...
func TestCheckHistoryStaleMiddleware(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			historyMiddleware := httpx.NewHistoryMiddleware(ledgerState, testCase.staleThreshold, tt.FrontierSession())
