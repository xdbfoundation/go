...
type frontierChangeProcessor interface {
	io.ChangeProcessor
	Commit() error
}

type frontierTransactionProcessor interface {
...
func (s *ProcessorRunner) buildChangeProcessor(
	...
	return newGroupChangeProcessors([]frontierChangeProcessor{
...
func (s *ProcessorRunner) buildTransactionProcessor(
	...
	return newGroupTransactionProcessors([]frontierTransactionProcessor{
...
// checkIfProtocolVersionSupported checks if this Frontier version supports the
...
func (s *ProcessorRunner) checkIfProtocolVersionSupported(ledgerSequence uint32) error {
	...
	if ledgerVersion > MaxSupportedProtocolVersion {
		return fmt.Errorf(
			"This Frontier version does not support protocol version %d. "+
				"The latest supported protocol version is %d. Please upgrade to the latest Frontier version.",
			ledgerVersion,
...
// matches the one in corresponding ledger header in digitalbits-core backend.
// This gives you full security if data in digitalbits-core backend can be trusted
...
func (s *ProcessorRunner) validateBucketList(ledgerSequence uint32) error {
	...
	if !exists {
		return fmt.Errorf(
			"cannot validate bucket hash list. Checkpoint ledger (%d) must exist in DigitalBits-Core database.",
			ledgerSequence,
		)
	}
...
func (s *ProcessorRunner) runChangeProcessorOnLedger(
	changeProcessor frontierChangeProcessor, ledger uint32,
) error {
	var changeReader io.ChangeReader
	var err error
