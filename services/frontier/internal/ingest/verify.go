...
func (s *system) verifyState(verifyAgainstLatestCheckpoint bool) error {
	...
	if verifyAgainstLatestCheckpoint {
		...
		for {
			...
			// Frontier is catching up. It doesn't make sense to verify old ledgers as
			...
			localLog.Info("Waiting for digitalbits-core to publish HAS...")
			select {
			case <-s.ctx.Done():
				localLog.Info("State verifier shut down...")
				return nil
			case <-time.After(5 * time.Second):
				// Wait for digitalbits-core to publish HAS
				retries++
				if retries == 12 {
