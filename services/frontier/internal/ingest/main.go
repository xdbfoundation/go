...
const (
	...
	// MaxDBConnections is the size of the postgres connection pool dedicated to Frontier ingestion:
	//  * Ledger ingestion,
	//  * State verifications,
	//  * Metrics updates.
	MaxDBConnections = 3

	defaultCoreCursorName           = "FRONTIER"
...
type system struct {
	...
	digitalbitsCoreClient digitalbitsCoreClient
...
func NewSystem(config Config) (System, error) {
		...
		if len(config.RemoteCaptiveCoreURL) > 0 {
			...
			ledgerBackend, err = ledgerbackend.NewCaptive(
				ledgerbackend.CaptiveCoreConfig{
					...
					CheckpointFrequency: config.CheckpointFrequency,
					LedgerHashStore:     ledgerbackend.NewFrontierDBLedgerHashStore(config.HistorySession),
					Log:                 log.WithField("subservice", "digitalbits-core"),
					Context:             ctx,
				},
			)
			if err != nil {
	...
	system := &system{
		...
		digitalbitsCoreClient: &digitalbitscore.Client{
			URL: config.DigitalBitsCoreURL,
		},
...
func (s *system) initMetrics() {
	s.metrics.LedgerIngestionDuration = prometheus.NewSummary(prometheus.SummaryOpts{
		Namespace: "frontier", Subsystem: "ingest", Name: "ledger_ingestion_duration_seconds",
	...
	s.metrics.StateVerifyDuration = prometheus.NewSummary(prometheus.SummaryOpts{
		Namespace: "frontier", Subsystem: "ingest", Name: "state_verify_duration_seconds",
		Help: "state verification durations, sliding window = 10m",
	})

	s.metrics.StateInvalidGauge = prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			Namespace: "frontier", Subsystem: "ingest", Name: "state_invalid",
	...
	s.metrics.LedgerStatsCounter = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "frontier", Subsystem: "ingest", Name: "ledger_stats_total",
	...
	s.metrics.ProcessorsRunDuration = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Namespace: "frontier", Subsystem: "ingest", Name: "processor_run_duration_seconds_total",
...
// that means that Frontier ingestion can be running on multiple machines and
...
// of order book graph in memory of each Frontier instance. To solve this:
// * For state init:
	...
	for {
		...
		if err != nil {
			...
			if isCancelledError(err) {
				// We only expect context.Canceled errors to occur when frontier is shutting down
...
func (s *system) updateCursor(ledgerSequence uint32) error {
	if s.digitalbitsCoreClient == nil || s.config.EnableCaptiveCore {
		return nil
	}

	cursor := defaultCoreCursorName
	if s.config.DigitalBitsCoreCursor != "" {
		cursor = s.config.DigitalBitsCoreCursor
	}

	ctx, cancel := context.WithTimeout(s.ctx, time.Second)
	defer cancel()
	err := s.digitalbitsCoreClient.SetCursor(ctx, cursor, int32(ledgerSequence))
	if err != nil {
		return errors.Wrap(err, "Setting digitalbits-core cursor failed")
	}
