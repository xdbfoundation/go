...
var dbCmd = &cobra.Command{
	Use:   "db [command]",
	Short: "commands to manage frontier's postgres db",
}
...
var dbInitCmd = &cobra.Command{
	...
	Long:  "init initializes the postgres database used by frontier.",
	Run: func(cmd *cobra.Command, args []string) {
		requireAndSetFlag(frontier.DatabaseURLFlagName)
...
var dbReapCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		app := frontier.NewAppFromFlags(config, flags)
...
var reingestRangeCmdOpts = []*support.ConfigOption{
	{
		...
		Required:    false,
		FlagDefault: false,
		Usage: "[optional] if this flag is set, frontier will be blocked " +
	...
	},
	{
		...
		Required:    false,
		FlagDefault: uint(1),
		Usage:       "[optional] if this flag is set to > 1, frontier will parallelize reingestion using the supplied number of workers",
	},
	{
...
var dbReingestRangeCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		frontier.ApplyFlags(config, flags)
		...
		if err != nil {
			if errors.Cause(err) == ingest.ErrReingestRangeConflict {
				message := `
			The range you have provided overlaps with Frontier's most recently ingested ledger.
			It is not possible to run the reingest command on this range in parallel with
			Frontier's ingestion system.
			Either reduce the range so that it doesn't overlap with Frontier's ingestion system,
			or, use the force flag to ensure that Frontier's ingestion system is blocked until
...
func RunDBReingestRange(from, to uint32, reingestForce bool, parallelWorkers uint, config frontier.Config) error {
	if reingestForce && parallelWorkers > 1 {
		return errors.New("--force is incompatible with --parallel-workers > 1")
	}
	frontierSession, err := db.Open("postgres", config.DatabaseURL)
	if err != nil {
		return fmt.Errorf("cannot open Frontier DB: %v", err)
	}

	ingestConfig := ingest.Config{
		NetworkPassphrase:           config.NetworkPassphrase,
		HistorySession:              frontierSession,
	...
	if !ingestConfig.EnableCaptiveCore {
		if config.DigitalBitsCoreDatabaseURL == "" {
			return fmt.Errorf("flag --%s cannot be empty", frontier.DigitalBitsCoreDBURLFlagName)
		}
		coreSession, dbErr := db.Open("postgres", config.DigitalBitsCoreDatabaseURL)
		if dbErr != nil {
